<h1 align="center">Hi there👋, I'm Chong Liu</h1>
<p align="center">A passionate roboticist</p>

<br>

- 🔭 I’m currently working on customized motion control solutions for humanoid robots at [BridgeDP Robotics](https://imchong.github.io/experience/bridgedp.html), alongside [Humanoid Robot Learning Paper Notebooks](https://imchong.github.io/Humanoid_Robot_Learning_Paper_Notebooks/), [Robotics Tech Stack](https://imchong.github.io/Robotics_Notebooks/), and [RL Sim2Sim Demo Website](https://imchong.github.io/RL_Sim2Sim_Demo_Website/).

- 🌱 I’m currently learning [mjlab](https://github.com/mujocolab/mjlab) and [MuJoCo](https://mujoco.org/) for GPU-accelerated humanoid robot reinforcement learning.

- 👨‍💻 All of my projects are available at [Home | Chong Liu](https://imchong.github.io/).

- 💬 Ask me about humanoid robot motion control, reinforcement learning, imitation learning, Sim2Real, C/C++/Python, embedded software, and the robotics development process.

- 📫 Reach me at <chongliu2021@163.com> or drop an issue / discussion in the corresponding repositories.

- 💡 As a lifelong learner, it's a great pleasure for me to share my knowledge.

<br>
